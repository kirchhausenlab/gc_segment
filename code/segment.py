...
class SegmentationModule(object):
    def __init__(
        ...
        assert os.path.exists(self.anno_dir), 'No annotations found in {}. Please annotate first!'.format(self.anno_dir)
        ...
        assert os.path.exists(self.data_path), 'Specified data_path {} does not exist!'.format(self.data_path)
    
    
        # ====================================================================
        #   Print configuration hyperparams. 
        write_flush('\nThe configuration of the experiment is: \n')
        write_flush('{:>30} : {}\n'.format('Configuration file', os.path.abspath(cfg)))
    ...
    def __call__(
        ...
        if self.labels1 is None:
            ...
            self.labels1            = sksegmentation.slic(rgb, 
                                                              ...
                                                              multichannel=True)
        write_done()
        print('Size of the volume is Z: %d, Y: %d, X: %d'%(self.labels1.shape[0],\
    ...
    #    HTML(anim.to_html5_video())
        ...
        if self.grey_v_px is None:
            self.grey_v_px               = get_grey_values(image, self.labels1, n_jobs=self.options.n_jobs)
        write_done()
        # ====================================================================
        ...
        if self.spx_locations is None:
            ...
            spx_x_coord             = get_grey_values(X_, self.labels1, n_jobs=self.options.n_jobs)
            write_done()
            align_left('Compting y-coordinate of centres of supervoxels')
            spx_y_coord             = get_grey_values(Y_, self.labels1, n_jobs=self.options.n_jobs)
            write_done()
            align_left('Compting z-coordinate of centres of supervoxels')
            spx_z_coord             = get_grey_values(Z_, self.labels1, n_jobs=self.options.n_jobs)
            write_done()
        ...
        for f_ in anno_files:
            ...
            anno_plane              = int(os.path.split(f_path)[-1].split('.')[0].split('_')[1])
            anno_img                = imageio.imread(f_path) / 255
            
            if any([k in f_path for k in ['Foreground', 'Object']]) and anno_plane < image.shape[0]:
    ...
    #            raise ValueError('Could not understand annotation file %s' %(f_path))
        ...
        fg_inv_prob_spx             = np.log(1 + get_grey_values(fg_inv_prob, \
                                                                self.labels1, \
                                                                n_jobs=self.options.n_jobs))
        bg_inv_prob_spx             = np.log(1 + get_grey_values(bg_inv_prob, \
                                                                self.labels1, \
                                                                n_jobs=self.options.n_jobs))
        ...
        pw_edges, pw_costs          = get_pairwise_costs(self.grey_v_px, self.spx_locations, \
                                        self.adj_mat, self.beta, image.shape[:3], sigma=50)
        ...
        #                     cells_per_block=(1, 1), visualize=True, feature_vector=False, multichannel=False)
        ...
        print('{:<80}{:.4f}, {:.4f}'.format('Limits of histogram-based foreground unaries', fg_unaries.min(), fg_unaries.max()))
        print('{:<80}{:.4f}, {:.4f}'.format('Limits of histogram-based background unaries', bg_unaries.min(), bg_unaries.max()))
        print('{:<80}{:.4f}, {:.4f}'.format('Limits of distance-based foreground unaries', fg_inv_prob_spx.min(), fg_inv_prob_spx.max()))
        print('{:<80}{:.4f}, {:.4f}'.format('Limits of distance-based background unaries', bg_inv_prob_spx.min(), bg_inv_prob_spx.max()))
        print('{:<80}{:.4f}, {:.4f}'.format('Limits of pairwise terms', pw_costs.min(), pw_costs.max()))
        ...
        try:
            if self.options.use_gmm:
                ...
                for gmmc in range(self.options.n_gmm_components):
                    mask_path_          = os.path.join(self.mask_dir, 'mask%d/' %(gmmc))
                ...
                for s in range(new_fg_mask.shape[0]):
                    for gmmc in range(self.options.n_gmm_components):
                        mask            = (new_fg_mask[s,:,:] == (1 + gmmc)).astype(np.uint8)
                        mask_img[:,:]   = mask * 255
                        seg_img[:,:]    = mask * image[s,:,:]
                        imageio.imsave(os.path.join(cropped_mask_paths[gmmc], orig_file_list[s+z_limits[0]]), mask_img)
                        imageio.imsave(os.path.join(cropped_seg_paths[gmmc], orig_file_list[s+z_limits[0]]), seg_img)
    
                    imageio.imsave(os.path.join(self.data_dir, orig_file_list[s+z_limits[0]]), image[s,:,:])
    ...
    #                oimg                = imageio.imread(os.path.join(self.data_path, orig_file_list[s+z_limits[0]]))
                    ...
                    imageio.imsave(os.path.join(cropped_mask0_path, orig_file_list[s+z_limits[0]]), mask_img)
                    imageio.imsave(os.path.join(cropped_seg0_path, orig_file_list[s+z_limits[0]]), seg_img)
                    imageio.imsave(os.path.join(self.data_dir, orig_file_list[s+z_limits[0]]), image[s,:,:])
        except Exception as E:
            write_fail()

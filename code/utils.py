...
def get_grey_values(I, L, n_jobs=32):
    # Get average grey values for every superpixel, and call it the grey value for the superpixel
...
def compute_beta(grey_values, adj_mat, n_jobs=32):
    ...
    with parallel_backend('threading', n_jobs=n_jobs):
        w_l_contr = Parallel()(delayed(compute_edge_contr)(m, grey_values, adj_mat[m,m+1:]) for m in range(n_superpixels-1))
...
def get_pairwise_costs(grey_values, px_locations, adj_mat, beta, imsize, sigma=10):
    edge_list = []
    n_superpixels = adj_mat.shape[0]
    ...
    for m in range(n_superpixels - 1):
        neighs = np.array([m + 1 + n for n in np.where(adj_mat[m, m+1:] == 1)[0]])
        
        xm, ym, zm = px_locations[m,0], px_locations[m,1], px_locations[m,2]
        
        for n in neighs:
            xn, yn, zn = px_locations[n,0], px_locations[n,1], px_locations[n,2]
            
            mult = np.exp(-beta * np.sum((grey_values[m] - grey_values[n])**2) / (2 * sigma**2)) 
            dist = np.sqrt((xm - xn) ** 2 + (ym - yn) ** 2 + (zm - zn) ** 2)
            assert dist > 0, 'dist = 0 for m = %d [%d, %d], n = %d [%d, %d]' %(m, ym, xm, n, yn, xn)
            cost = mult * (1. / dist)

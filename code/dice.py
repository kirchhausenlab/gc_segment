...
def main(sys_string=None):
    ...
    parser.add_argument('--true', type=str, default=None, help='Path to ground truth')
    parser.add_argument('--pred', type=str, default=None, help='Path to prediction')
    parser.add_argument('--z_limits', type=int, nargs='+', default=None, help='Optional limits on the Z.')
    ...
    assert os.path.exists(args.true), 'Ground truth path does not exist. Got {}'.format(args.true)
    assert os.path.exists(args.pred), 'Prediction path does not exist. Got {}'.format(args.pred)
